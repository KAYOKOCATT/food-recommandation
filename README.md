...
当前路线是：中文菜品做统计推荐和演示型收藏协同过滤，Yelp 餐厅做内容推荐和邻域协同过滤，实时推荐只做基于最近行为的离线候选重排。Yelp v1 已支持独立数据入库、餐厅列表/详情页，以及基于离线内容相似度的相似餐厅展示。当前实现还补上了 Yelp 餐厅详情页的站内评分/评论入口，以及基于 `YelpReview(stars)` 的 Yelp UserCF 离线召回命令。
...
餐厅列表 / 详情 ORM 查询         build_yelp_content_recs      build_yelp_review_usercf
        ...
        +------------------------------+-----------------------------+
                     ...
                     apps.recommendations.services.YelpService
                                       |
                                       v
         Yelp 餐厅列表页 / 餐厅详情页 / 相似餐厅推荐 / 预留“为你推荐”入口
```
...
4. Yelp 餐厅协同过滤：基于 review 评分构造 user-business 矩阵，实现 UserCF/ItemCF 这类邻域算法，不把矩阵分解作为 v1 主线。当前已补齐评分版 UserCF 的离线构建命令。
...
- `data/recommendations/yelp_usercf.json` 负责 Yelp 用户到候选餐厅的离线 UserCF 结果。
...
Yelp 餐厅评分版 UserCF 命令：

```bash
# 从数据库中的 YelpReview(stars) 构建用户到餐厅的离线 UserCF JSON
python manage.py build_yelp_review_usercf --top-k 20

# 调整活跃度阈值、相似用户数和共同评分阈值
python manage.py build_yelp_review_usercf --similar-user-k 30 --min-user-reviews 5 --min-business-reviews 10 --min-common-items 2
```
...
- `data/recommendations/yelp_usercf.json`：Yelp 用户到候选餐厅的离线 UserCF 结果。
...
4. 如需重建评分版 UserCF，再运行 `python manage.py build_yelp_review_usercf --top-k 20`
...
- Yelp 用户登录：计划增加单独的“演示型免密登录”入口，从已导入的 `source="yelp"` 用户中选择账号后直接建立 session，用于展示 Yelp 推荐效果，不与本地用户名密码登录混用。
- 管理员登录：计划增加独立管理员演示入口，固定使用 `User` 表第一条记录作为管理员身份来源。
- 管理员面板：计划采用项目内独立后台，而不是把 Django Admin 作为主展示入口；第一版覆盖业务表 CRUD，包括 `User`、`Foods`、`Collect`、`Comment`、`YelpBusiness`、`YelpReview`。
